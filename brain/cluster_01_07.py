...
#    list_path = [convert_unix_to_windows_path(i) for i in list_path]
    ...
    cwd = os.getcwd()
    species  = get_species_from_cwd()  
    # The species is obtained based on the folder name: H_ads stands for H adsorption on Ru45.


    # Get the Y value for GA model, adsorption at EF = 0       
    EF='0.0'
    if species in ['H', 'N', 'O']:
        gas_species = species + '2'
        print(gas_species)
        df_GA['Eads'] = df[EF] - gas_dict[gas_species]/2  -  gas_dict['slab']
    else:
        gas_species = species
        print(gas_species)
        df_GA['Eads'] = df[EF] - gas_dict[gas_species]  -  gas_dict['slab']
...
def bootstrap_cross_validation(models, X, Y, n_bootstrap=100):
    ...
    n_samples = int(len(X) * 0.90)  # 90% of the data for training
