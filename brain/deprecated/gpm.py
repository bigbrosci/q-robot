...
def get_coord_xyz(lines):
    ...
    coords = lines[line_start:line_end]
    return coords
...
def save_g4_gjf(coords, name):
    ...
    file_out.write('%schk=%s\n' %('%', chk_name))
    file_out.write('%smem=%s\n' %('%', mem))
    file_out.write('# %s trans=semidirect  scf=xqc\n' %(functional))
    file_out.write(' \n%s \n \n'     %(chk_name))
    file_out.write('%s %s\n' %(charge, multiplicity))
    file_out.writelines(coords)
    file_out.write('\n')
...
def save_poscar_from_extended_xyz(file_in):
    f_in  = open(file_in, 'r')
    ...
    cell = lines[1].rstrip().split('"')[1].split()  
    la = ' '.join(cell[:3])
    lb = ' '.join(cell[3:6])
    lc = ' '.join(cell[6:])
    ...
    out_name = 'POSCAR'
    file_out = open(out_name, 'w')
    file_out.write("Generated by Q-robot" + '\n' + '1.0 \n')
    file_out.write('%s\n%s\n%s\n' %(la, lb, lc))
    file_out.write(' '.join(dict_ele_coord.keys()) + '\n')
    file_out.write(' '.join([str(len(i)) for i in dict_ele_coord.values()]) + '\n')
    file_out.write('Selective dynamics\nCartesian\n')
...
def get_HG_dict_T(log_file, T, spin = 0):
    ...
    zpe = gr.read_zpe(log_file)/27.21352655601672 * converter   # zero point energy in hartree 
    E_zpe = gr.read_electronic_and_zpe(log_file) / 27.21352655601672 * converter  # ZPE corrected energy in hartree
...
def get_HG_dicts(log_file, spin=0):
    thermo_dicts = {}
    for Tem in range(50,1001,20):
        one_dict = {}
        dict_T = get_HG_dict_T(log_file, Tem)
        one_dict.update({'Cp': dict_T.get('Cp')})
        one_dict.update({'S': dict_T.get('S')})
        thermo_dicts.update({Tem:one_dict})   
    return thermo_dicts
...
def get_HG_dict_T_G4(log_file, T, spin = 0):
    ...
    Freq   = [ float(i) * 0.9854 for i in gr.read_frequencies(log_file)]  
...
def get_HG_dicts_G4(log_file, spin=0):
    '''Returns the Cp, S at Temperatures from 50 to 1500 by every 10 K '''
    thermo_dicts = {}
    for Tem in range(50,1001,20):
        one_dict = {}
        dict_T = get_HG_dict_T_G4(log_file, Tem)
        one_dict.update({'Cp': dict_T.get('Cp')})
        one_dict.update({'S': dict_T.get('S')})
        thermo_dicts.update({Tem:one_dict}) 
    return thermo_dicts
