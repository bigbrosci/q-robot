...
def check_jobs():
    ...
    3) check the improved dimer jobs:
    3.1) if the job goes wrong, cancel it and save it to job_problems.csv file
    3.2) otherwise, keep running or waiting.
     '''
    ...
    for job_id, job_dir in id_dict.items():
        ...
        # if job_id in list_j: # jobs in the queue
            ...
            if os.path.exists(outcar):
                ...
                '''The job has not been started yet because the job is still in the queue and  UTCAR ii not found in the path. '''
                pass
